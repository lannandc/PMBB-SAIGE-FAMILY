...
def make_output_file_names(is_phewas, is_gene_burden, is_rare_vars, arg_dict):
    ...
    if not is_phewas and is_gene_burden and not is_rare_vars:
        ...
        output_filenames['merge'] = f'{pheno}.exwas_regions.saige.gz'
        output_filenames['filter'] = f'{pheno}.exwas_regions.filtered.saige.csv'
        output_filenames['merge_cauchy'] = f'{pheno}.exwas_cauchy_tests.saige.gz'
        output_filenames['merge_filter'] = f'{pheno}.exwas_cauchy_tests.filtered.saige.csv'
    elif not is_phewas and not is_gene_burden and not is_rare_vars:
        # GWAS
        pheno = arg_dict.pheno
        output_filenames['merge'] = f'{pheno}.gwas.saige.gz'
        output_filenames['filter'] = f'{pheno}.gwas.filtered.saige.csv'
    elif is_phewas and is_gene_burden and not is_rare_vars:
        # Gene burden PheWAS
        chrom = arg_dict.chr
        output_filenames['merge'] = f'chr{chrom}.gene_phewas_regions.saige.gz'
        output_filenames['filter'] = f'chr{chrom}.gene_phewas_regions.filtered.saige.csv'
        output_filenames['merge_cauchy'] = f'chr{chrom}.gene_phewas_cauchy_tests.saige.gz'
        output_filenames['merge_filter'] = f'chr{chrom}.gene_phewas_cauchy_tests.filtered.saige.csv'
    elif is_phewas and not is_gene_burden and not is_rare_vars:
        # Variant PheWAS
        chrom = arg_dict.chr
        output_filenames['merge'] = f'chr{chrom}.variant_phewas.saige.gz'
        output_filenames['filter'] = f'chr{chrom}.variant_phewas.filtered.saige.csv'
    elif not is_phewas and not is_gene_burden and is_rare_vars:
        # Single rare-variant tests accompanying ExWAS
        pheno = arg_dict.pheno
        output_filenames['merge'] = f'{pheno}.exwas_singles.saige.gz'
        output_filenames['filter'] = f'{pheno}.exwas_singles.filtered.saige.csv'
        output_filenames['merge_UR'] = f'{pheno}.exwas_ultrarare_tests.saige.gz'
        output_filenames['filter_UR'] = f'{pheno}.exwas_ultrarare_tests.filtered.saige.csv'
    elif is_phewas and not is_gene_burden and is_rare_vars:
        # Single rare-variant tests accompanying Gene burden PheWAS
        chrom = args.chr
        output_filenames['merge'] = f'chr{chrom}.gene_phewas_singles.saige.gz'
        output_filenames['filter'] = f'chr{chrom}.gene_phewas_singles.filtered.saige.csv'
        output_filenames['merge_UR'] = f'chr{chrom}.gene_phewas_ultrarare_tests.saige.gz'
        output_filenames['filter_UR'] = f'chr{chrom}.gene_phewas_ultrarare_tests.filtered.saige.csv'
...
for f in input_files:
    ...
    f_no_suffix = f.replace('.singleAssoc', '').replace('.txt', '')
    f_no_chr = '.'.join(f_no_suffix.split('.')[:-1])
    f_pheno = f_no_chr.split('/')[-1]
    temp = pd.read_table(f) # Read results file with default behaviors
...
for df, out, out_filtered in output_groups:
    ...
    if cc_thresh:
        ...
        df_filtered = df[(p_value_col <= p_thresh) & (case_col_value >= cc_thresh) & (control_col_value >= cc_thresh) ]
        
        
    else:

        df_filtered = df[(p_value_col <= p_thresh)]
        
    df_filtered.insert(0, 'cohort', cohort)
    # if not is_phewas:
    #     df_filtered.insert(1, 'phenotype', df_filtered['PHENO'])
